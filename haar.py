...
def wavelet_denoising_haar(x: torch.Tensor, tau: float, level: int) -> torch.Tensor:
    """
    ...
    for c in range(x.size(-1)):
        ...
        for j in range(len(detail_levels)):
            ...
            # N_j is the total number of coefficients in all subbands at this level
            # We assume all subbands at this level have the same shape/N for simplicity in 3D DWT
            N_j = detail_tuple[0].size
            
            # Universal Threshold formula: sigma_n * sqrt(2 * log(N_j))
            universal_lambda = sigma_n * np.sqrt(2.0 * np.log(N_j))
            
            # The actual threshold used, scaled by the input tau (now a scaling factor)
            lambda_j = universal_lambda * tau 

            # Apply soft thresholding to each of the three detail subbands (H, V, D)
            for detail_subband in detail_tuple:
                new_detail_tuple.append(soft_threshold(detail_subband, lambda_j))
